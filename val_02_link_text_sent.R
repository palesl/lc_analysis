...
flan_out   <- readRDS("data/text/text_04_valence_sentiment.rds")
...
flan_sum<- flan_linked|>
  mutate(yes_dummy=case_when(flan_class=="No"~0,flan_class=="Yes"~1))|>
  group_by(austlii_id, parliament)|>
  summarise(n_refs = n(),
            flan_count = sum(yes_dummy),
            flan_mean = mean(yes_dummy),
            flan_sd = sd(yes_dummy),
            flan_sent = mean(sentiment_valence),
            flan_sent_relevant = mean(sentiment_valence[flan_class==1])
  )
...
out_flan_sum<-flan_sum|>left_join(austlii_linked)|>
  select(id, austlii_id, parliament:flan_sent_relevant)
...
text_verif_merged|>group_by(principal_name)|>
  ...
  ggplot()+
  geom_line(aes(parliament,flan_sent))+
  geom_point(aes(parliament,flan_sent))+
...
text_verif_merged|>group_by(parliament,subject)|>
  summarise(mean_flan = mean(flan_sent),
